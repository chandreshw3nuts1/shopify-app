...
export const loader = async ({ params, request }) => {
	try {
		...
		return { reviewId, shopRecords, generalAppearancesModel, customQuestionsData, productReviewsModel, StarIcon, discountObj, translations, reviewFormSettingsModel, languageWiseReviewFormSettings, generalSettingsModel };
	} catch (error) {
		console.log(error);
	}
	return {};
...
const ResendReviewRequestForm = () => {
	const { reviewId, shopRecords, generalAppearancesModel, customQuestionsData, productReviewsModel, StarIcon, discountObj, translations, reviewFormSettingsModel, languageWiseReviewFormSettings, generalSettingsModel } = useLoaderData();
	if (!productReviewsModel) {
	...
	const [firstName, setFirstName] = useState(productReviewsModel?.first_name || "");
	const [lastName, setLastName] = useState(productReviewsModel?.last_name || "");

	const [email, setEmail] = useState(productReviewsModel?.email || "");
	...
	useEffect(() => {
		...
		if (productReviewsModel.is_resend_review_submitted) {
	...
	const submitReview = async () => {
		...
		if (validationErrors.firstName == '' && validationErrors.lastName == '' && validationErrors.email == '') {
			...
			formData.append('variant_title', productReviewsModel.variant_title ?? '');
			formData.append('rating', rating);
			formData.append('file_objects', uploadedDocuments);
			formData.append('description', reviewDescription);
			formData.append('reviewId', reviewId);
			formData.append('customer_locale', productReviewsModel?.customer_locale);
...
export default ResendReviewRequestForm;
