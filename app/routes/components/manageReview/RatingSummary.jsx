...
const RatingSummary = ({ ratingData}) => {
	...
	ratingData.forEach(item => {
		if(item.stars == 5){
			five_start_percent = (item.count /totalReviews) * 100;
		} else if(item.stars == 4) {
			four_start_percent = (item.count /totalReviews) * 100;
		} else if(item.stars == 3) {
			three_start_percent = (item.count /totalReviews) * 100;
		} else if(item.stars == 2) {
			two_start_percent = (item.count /totalReviews) * 100;
		} else if(item.stars == 1) {
			one_start_percent = (item.count /totalReviews) * 100;
		}
	});
  return (
