if (typeof jQuery !== 'undefined') {
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.owl-carousel .play-pause', function () {
            const $carousel = $(this).closest('.owl-carousel'); // Target specific carousel instance
            const video = $(this).closest('.item').find('.video-content')[0];
            const videoDiv = $(this).closest('.item');

            // Pause all other videos within the same carousel
            $carousel.find('.video-content').each(function () {
                if (this !== video) {
                    this.pause();
        ...
        $(document).on('click', '.reviewpopupbtn', function (event) {
            $(this).closest('.video-div').find('video').each(function() {
                this.pause();
                $(this).html('<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.5 8.98505V6.48882C3.5 3.37974 5.65027 2.1092 8.28325 3.66374L10.4043 4.91933L12.5253 6.17492C15.1582 7.72946 15.1582 10.2705 12.5253 11.8251L10.4043 13.0807L8.28325 14.3363C5.65027 15.8908 3.5 14.6203 3.5 11.5112V8.98505Z" fill="currentColor"/></svg>'); // Play Icon

            });
            $(this).closest('.video-div').find('.playpause').html('<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.5 8.98505V6.48882C3.5 3.37974 5.65027 2.1092 8.28325 3.66374L10.4043 4.91933L12.5253 6.17492C15.1582 7.72946 15.1582 10.2705 12.5253 11.8251L10.4043 13.0807L8.28325 14.3363C5.65027 15.8908 3.5 14.6203 3.5 11.5112V8.98505Z" fill="currentColor"/></svg>'); // Play Icon
        });
